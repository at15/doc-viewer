...
function renderNode(nodeName, node) {
    ...
    if (node._hasChild) {
        ...
        if(nodeName === ''){
            // TODO:move the attributes to front end code.
            s = '<ul class="nav" data-spy="affix" data-offset-top="195">' + s + '</ul>';
        }else{
            s = '<a href="#' + nodeName + '">' + nodeName + '</a>' + '<ul>' + s + '</ul>';
        }
    } else {
        s = '<a href="#' + nodeName + '">' + nodeName;
    }
    return s;
}
